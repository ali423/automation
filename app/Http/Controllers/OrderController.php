...
class OrderController extends Controller
{
    ...
    public function getChartData(Request $request)
    {
        try {
            ...
            return response()->json([
                'error' => 'خطا در پردازش اطلاعات نمودار',
    ...
    {
        try {
            \Log::info('Getting commodity units', ['commodity_id' => $commodityId]);
            
            $commodity = Commodity::with(['unit', 'unitConversions.fromUnit', 'unitConversions.toUnit'])->findOrFail($commodityId);
            
            \Log::info('Found commodity', [
                'commodity_id' => $commodity->id,
                'commodity_title' => $commodity->title,
                'has_unit' => $commodity->unit ? 'yes' : 'no',
                'unit_name' => $commodity->unit ? $commodity->unit->name : 'none'
            ]);
            
            $selectableUnits = $this->commodityUnitService->getSelectableUnits($commodity);
            
            \Log::info('Selectable units', [
                'commodity_id' => $commodityId,
                'units_count' => $selectableUnits->count(),
            ...
            \Log::error('Error getting commodity units', [
                'commodity_id' => $commodityId,
                'error' => $e->getMessage(),
            ...
            return response()->json([
                'success' => false,
                'message' => 'خطا در دریافت واحدهای کالا',
                'units' => []
            ], 500);
        }
    }
