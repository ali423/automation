...
@section('title', __('fields.add_conversion'))

@section('content')
    <div class="row">
        <div class="col-xl-12 box-margin height-card">
            <div class="card card-body">
                <h4 class="card-title">{{ __('fields.add_conversion') }}</h4>
                <div class="row">
                    <div class="col-sm-12 col-xs-12">
                        <form method="post" action="{{ route('unit-conversion.store') }}" class="needs-validation" novalidate>
                            @csrf
                            <div class="form-row">
                                <div class="form-group col-md-4">
                                    ...
                                    <select name="commodity_id" id="commodity_id" class="form-control @error('commodity_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($commodities as $commodity)
                                            <option value="{{ $commodity->id }}" data-unit-id="{{ $commodity->unit_id }}" data-unit-name="{{ $commodity->unit ? $commodity->unit->name : '' }}" data-unit-symbol="{{ $commodity->unit ? $commodity->unit->symbol : '' }}" {{ old('commodity_id') == $commodity->id ? 'selected' : '' }}>
                                                {{ $commodity->title }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <button type="submit" class="btn btn-primary mr-2">{{ __('fields.add_conversion') }}</button>
