                                ...
                                @foreach($request->commodities as $request_commodity)
                                    <div id="inputFormRow" class="form-row shadow p-4 mb-3">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <select id="commodity_id" class="form-control commodity-select" name="commodity_id[]"
                                                    onchange="pricefunc(this)" required>
                                                <option value="">انتخاب کنید</option>
                                                @foreach ($commodities as $commodity)
                                                    <option value="{{ $commodity->id }}" data-attributes="{{ $commodity->attributes->pluck('id')->join(',') }}"
...
@section('page_scripts')
    <script type="text/javascript">
        ...
        @if(isset($attributes) && $attributes->count() > 0)
        var attributeFilterHtml = '<div class="col-12 mb-2 attribute-filter-container"><div class="d-inline-flex align-items-center" style="cursor: pointer;" onclick="$(this).closest(\'.attribute-filter-container\').find(\'.filter-panel\').slideToggle(200);"><i class="ti-filter toggle-row-filter" style="font-size: 12px; color: #666;"></i><small style="margin-right: 6px; font-size: 11px; color: #333;">فیلتر ویژگی</small></div><div class="filter-panel mt-2" style="display: none;"><div class="card" style="background-color: #f8f9fa; border: 1px solid #e0e0e0;"><div class="card-body p-2"><div class="d-flex justify-content-between align-items-center mb-2"><input type="text" class="form-control form-control-sm row-attribute-search" placeholder="جستجو..." style="font-size: 12px; width: 150px;"><button type="button" class="btn btn-xs btn-secondary clear-row-filters" style="font-size: 11px; padding: 2px 8px;">پاک کردن</button></div><div class="d-flex flex-wrap gap-1 row-attribute-filters" style="max-height: 150px; overflow-y: auto; scrollbar-width: thin;">@foreach($attributes as $attribute)<button type="button" class="btn btn-xs btn-outline-primary row-attribute-filter" data-attribute-id="{{ $attribute->id }}" data-name="{{ $attribute->name }}" style="font-size: 11px; padding: 2px 8px; margin: 2px;">{{ $attribute->name }}</button>@endforeach</div></div></div></div></div>';
        @else
