    ...
    <div class="card card-body">
        <div class="row">
            <div class="col-sm-12 col-xs-12">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>فروشنده: <span>{{ $request->seller ? $request->seller->name : 'نامشخص' }}</span></div>
                ...
                <div class="mb-3">
                    <div class="row">
                        <div class="col-md-6">
                            <strong>استان:</strong> <span>{{ $request->seller ? $request->seller->province : '' }}</span>
                        </div>
                        <div class="col-md-6">
                            <strong>شهر:</strong> <span>{{ $request->seller ? $request->seller->city : '' }}</span>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody>
                            ...
                            @foreach($request->commodities as $commodity)
                                <tr>
                                    ...
                                    @if($invoiceType === 'documentation')
                                        <td>{{ isset($commodity->pivot->purchase_price) ? number_format($commodity->pivot->purchase_price) : '-' }}</td>
                                        <td>{{ isset($commodity->pivot->purchase_price) ? number_format($commodity->pivot->amount * $commodity->pivot->purchase_price) : '-' }}</td>
                                    @endif
                                </tr>
                                @php($i++)
                            @endforeach
                            <tr>
                                <td colspan="{{ $invoiceType === 'documentation' ? '7' : '5' }}" class="text-right">
                                    ...
                                    @if($invoiceType === 'documentation')
                                        <br>مجموع: {{ $request->commodities->sum(function($commodity) { return $commodity->pivot->amount * ($commodity->pivot->purchase_price ?? 0); }) }}
                                    @endif
                                </td>
                            </tr>
                        </tbody>
